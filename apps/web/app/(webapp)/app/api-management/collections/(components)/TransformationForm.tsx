...
const TransformationForm = ({ rawData, profileData, preview }: APIConfigurationProps) => {
  ...
  const incorrect = !request_body || !response_body;
  ...
  return (
    <>
      {open2FA && (
        <AppCenterModal title={"Two-Factor Authentication"} effect={close2FAModal}>
          <TwoFactorAuthModal close={close2FAModal} loading={loading} next={(value: string) => handle2FA(value)} />
        </AppCenterModal>
      )}

      {(openModal == "enable2fa" || openModal == "backupcode") && (
        <AppCenterModal
          title={openModal == "enable2fa" ? "Enable Two-Factor Authentication" : "Two-Factor Authentication Enabled"}
        ...
        >
          {openModal == "enable2fa" ? (
            ...
            <TwoFactoAuthEnabled backup_codes={backup_codes} close={handleCloseModal} />
          )}
        </AppCenterModal>
      )}
      ...
      <form onSubmit={(e) => handleSubmit(e, "")} className="w-full">
        <div className="flex items-start w-full justify-between gap-[40px] pb-[20px]">
          <div className="w-[40%] text-f14 font-[500] text-o-text-dark">Transformation</div>

          <div className="flex bg-white rounded-[12px] border border-o-border gap-[16px] flex-col px-[24px] pt-[10px] pb-[24px] w-[60%]">
            <div className="w-full">
              <ListPanel panel={paths} currentValue={path} setCurrentValue={setPath} />
            </div>

            {path == "request_body" && (
              <div className="w-full">
                <CodeEditor code={request_body} mode="lua" setCode={userType == "api-consumer" ? () => null : setRequestBody} />
              </div>
            )}

            {path == "response_body" && (
              <div className="w-full">
                <CodeEditor code={response_body} mode="lua" setCode={userType == "api-consumer" ? () => null : setResponseBody} />
              </div>
            )}

            {!previewPage && (
              <div className="w-full flex justify-end">
                <Button title="Save changes" type="submit" containerStyle="!w-[120px]" disabled={incorrect} small />
              </div>
            )}
          </div>
        </div>
