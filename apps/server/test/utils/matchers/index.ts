...
export const entityMatchers = {
  toBeValidEntity<T extends object>(
    ...
    const datePass = received?.createdAt instanceof Date || 
                   received?.created_at instanceof Date ||
                   received?.updatedAt instanceof Date;
    
    const typePass = !expectedType || 
    ...
    return {
      ...
      message: () => {
        ...
        if (!typePass && expectedType) missing.push(`type ${expectedType.name}`);
  ...
  toBePaginatedResponse(
    ...
    const paginationPatterns = [
      ...
      () => Array.isArray(received?.items) && typeof received?.meta?.total === 'number',
      // Pattern 3: NestJS-style with ResponseFormatter
      () => Array.isArray(received?.data) && typeof received?.meta?.totalNumberOfRecords === 'number'
    ];
    ...
    return {
      pass,
      message: () => `Expected paginated response (data[]+total or items[]+meta.total), got ${this.utils.printReceived(received)}`
    };
  },
  ...
  toBeUuid(
    ...
    return {
      pass,
      message: () => `Expected UUID string, got ${this.utils.printReceived(received)}`
    };
  },
  ...
  toBeIsoDate(
    ...
    return {
      pass,
      message: () => `Expected ISO date string, got ${this.utils.printReceived(received)}`
    };
  }
};
...
export const paginationMatchers = {
  toBePaginatedResponse(
    ...
    const paginationPatterns = [
      ...
      () => Array.isArray(received?.results) && typeof received?.offset === 'number'
    ];
    ...
    return {
      pass,
      message: () => `Expected paginated response structure, got: ${this.utils.printReceived(received)}`
    };
  },
  ...
  toContainPagedEntity<T extends object>(
    ...
    const pass = Array.isArray(array) && array.some(item => item instanceof expectedType);

    return {
      pass,
      message: () => `Expected paginated response to contain ${expectedType.name}`
    };
  }
};
...
export const responseMatchers = {
  toBeResponseWithStatus(
    ...
    return {
      pass,
      message: () => `Expected response with status ${expectedStatus}, got ${received?.status}`
    };
  },
  ...
  toBeApiResponse<T extends object>(
    ...
    const typePass = !expectedType || 
                   ...
                   (Array.isArray(received?.data) && received.data.every((item: any) => item instanceof expectedType));

    return {
      pass: basePass && typePass,
      message: () => `Expected API response with ${expectedType?.name || 'any'} data`
    };
  },
  ...
  toHaveResponseMeta(
    ...
    const missingKeys = expectedMetaKeys.filter(key => !(key in received?.meta));
